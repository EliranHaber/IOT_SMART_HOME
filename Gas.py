...
clientname = "GAS_client-" + str(r)
GAS_topic = 'pr/home/5976397/gas'
...
class ConnectionDock(QDockWidget):
    ...
    def __init__(self, mc):
        ...
        self.ePublisherTopic.setText(GAS_topic)

        self.GasLevel = QLineEdit()
        self.GasLevel.setText('')

        formLayout = QFormLayout()
        formLayout.addRow("Connect to Broker", self.eConnectbtn)
        formLayout.addRow("Publish topic", self.ePublisherTopic)
        formLayout.addRow("Gas Level (%)", self.GasLevel)

        widget = QWidget(self)
        widget.setLayout(formLayout)
        self.setTitleBarWidget(widget)
        self.setWidget(widget)
        self.setWindowTitle("Gas Sensor Emulator")
...
class MainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.setGeometry(50, 500, 300, 150)
        self.setWindowTitle('Gas Sensor Emulator')
    ...
    def update_data(self):
        gas_level = random.randrange(1, 90)
        current_data = f"Gas Level: {gas_level} %"
        print('Publishing:', current_data)
        self.connectionDock.GasLevel.setText(str(gas_level))
        self.mc.publish_to(GAS_topic, current_data)

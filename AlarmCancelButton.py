...
# Global variable for client name
global clientname, CONNECTED
CONNECTED = False
r = random.randrange(1, 10000000)
clientname = "AlarmCancel_client-" + str(r)  # More relevant client name

alarm_cancel_topic = 'pr/home/6761513/alarm/cancel'  # MQTT topic for alarm cancellation


class Mqtt_client():
    def __init__(self):
        # MQTT variable definitions
        self.broker = ''
        self.topic = ''
        self.port = ''
        self.clientname = ''
    ...
    def on_message(self, client, userdata, msg):
        ...
        # No UI update required for received message, but can be added if needed.

    def connect_to(self):
        # Creating Paho MQTT client instance
        self.client = mqtt.Client(client_id=clientname, clean_session=True)
        self.client.on_connect = self.on_connect  # Binding callback function
        self.client.on_disconnect = self.on_disconnect
        self.client.on_log = self.on_log
        self.client.on_message = self.on_message
        self.client.username_pw_set(self.username, self.password)
        print("Connecting to broker ", self.broker)
        self.client.connect(self.broker, self.port)  # Connecting to broker
...
class ConnectionDock(QDockWidget):
    ...
    def __init__(self, mc):
        ...
        self.eAlarmCancelBtn = QPushButton("Cancel Alarm", self)  # Alarm cancel button
        self.eAlarmCancelBtn.setToolTip("Click to cancel the alarm")
        self.eAlarmCancelBtn.clicked.connect(self.alarm_cancel_click)
        self.eAlarmCancelBtn.setStyleSheet("background-color: green")  # Green color as default
        ...
        formLayot.addRow("Cancel Alarm", self.eAlarmCancelBtn)  # Adding alarm cancel button
    ...
    def alarm_cancel_click(self):
        ...
        self.mc.publish_to(self.ePublisherTopic.text(), '{"alarm_status": "canceled"}')  # Publishing message
        print("Alarm cancellation message sent!")
        # You can add code here to change button color if you want visual indication.
        self.eAlarmCancelBtn.setStyleSheet("background-color: gray")  # For example, turning button gray after click
